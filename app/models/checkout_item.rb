class CheckoutItem < ApplicationRecord
  ...
  def self.top_items(number = nil, start_date = nil, end_date = nil)
    if start_date && end_date
      ...
      relation = CheckoutItem
		...
		top_items_and_counts = relation.group(:item_id).sum(:count).sort_by {|item_id, count| -count }
    top_items_and_counts = top_items_and_counts.first(number) if number
